...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the event of the player who is currently playing.
        """
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Return the updated GameState depending on if no move was made,
        a valid move was made, or an invalid move was made, in which case the
    ...
    def render(self, renderer: Renderer) -> None:
        """Render a board with turn, player, score, and the goal description.
        """
...
class AnimateMoveState(GameState):
    ...
    def update(self) -> GameState:
        """Return the updated GameState based on whether the animation is
        complete or still running. If the animation is complete, the last
        GameState will be shown; otherwise, the remain in current GameState.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        """Render a board with an outline around the selected block, draw the
        image representing the move and update the status message based on the
...
class GameOverState(GameState):
    ...
    def render(self, renderer: Renderer) -> None:
        """Render a screen showing the player id, goal, penalty, score and
        winner.
        """
        x = 10
        y = 10
